...
void _menu_move_distance(const AxisEnum axis, const screenFunc_t func, const int8_t eindex=-1) {
  ...
  if (LCD_HEIGHT >= 4) {
    switch (axis) {
      ...
      // Juraj case Z_AXIS: STATIC_ITEM(MSG_MOVE_Z, SS_DEFAULT|SS_INVERT); break;
...
void menu_move() {
  ...
  if (NONE(IS_KINEMATIC, NO_MOTION_BEFORE_HOMING) || all_axes_homed()) {
    ...
    #endif
    // Juraj SUBMENU(MSG_MOVE_Z, []{ _menu_move_distance(Z_AXIS, lcd_move_z); });
...
void menu_motion() {
  ...
  GCODES_ITEM(MSG_AUTO_HOME, G28_STR);
    ...
    // Juraj GCODES_ITEM(MSG_AUTO_HOME_Z, PSTR("G28Z"));
  ...
  //
    ...
    if (!g29_in_progress)
      // Juraj SUBMENU(MSG_BED_LEVELING, menu_bed_leveling);
